...
def verify_foreign_keys_exist(db, ref_id, labeler_id, raise_on_missing=False):
    ...
    if raise_on_missing and (not ref_exists or not labeler_exists):
    ...
    return bool(ref_exists), bool(labeler_exists)


def save_review_annotation(db, ref_id, labeler_id, annotations_str, unclear):
    verify_foreign_keys_exist(db, ref_id, labeler_id, raise_on_missing=True)
