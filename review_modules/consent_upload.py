...
def validate_consent_form_file(consent_form_file):
    ...
    filename = consent_form_file.filename
    if not filename or not filename.lower().endswith('.pdf'):
        return None, "Please upload a PDF file."
    
    if consent_form_file.content_type != 'application/pdf':
    ...
    if not consent_form_data or not consent_form_data.startswith(b'%PDF'):
...
def save_consent_form_to_filesystem(username, consent_form_data):
    try:
        ...
        import sys
        print(f"WARNING: Failed to write consent form to filesystem for {username}: {e}", file=sys.stderr)
        return None, str(e)
...
def process_consent_form_upload(review_conn, username, role, years_practicing, consent_form_file):
    ...
    if file_error:
        import sys
        print(f"WARNING: Consent form saved to database but failed to save to filesystem: {file_error}", file=sys.stderr)
