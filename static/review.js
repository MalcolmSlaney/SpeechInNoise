...
class SimpleWaveform {
  ...
  async loadWaveform() {
    ...
    try {
      ...
      // Create dummy data if Web Audio API fails
      this.waveformData = this.createDummyData();
      this.isLoaded = true;
      this.draw();
      
      // Notify that waveform has loaded (even if it's dummy data)
      if (this.onLoaded) {
        this.onLoaded();
      }
    } finally {
  ...
  extractWaveformFromBuffer(audioBuffer) {
    ...
    for (let i = 0; i < targetBars; i++) {
      ...
      // Apply mathematical weighting to make silences more obvious
      let weightedAmplitude = rms
      
      // Apply logarithmic scaling to emphasize differences
  ...
  draw() {
    ...
    // Draw playhead if there's progress
  ...
  updateProgress(currentTime, duration) {
    ...
    this.draw() // Redraw with updated playhead
  }
...
// ReviewAudio extends AudioPrefetch directly (like Audio in record.js does)
// This gives us the prefetching and audio loading functionality
...
class ReviewAudio extends AudioPrefetch {
  constructor(project) {
    console.log('ReviewAudio constructor called with project:', project)
    // Pass "#playing" as the audio selector, and project as the second argument
    // This matches how the Audio class in record.js does it: super("#playing", project)
    console.log('ReviewAudio calling super("#playing", project)')
    super("#playing", project)
    console.log('ReviewAudio super() completed')
    ...
    if (window.ReviewModules) {
      ...
      this.buttonManager = new window.ReviewModules.ButtonManager(this.dom, this.ui);
      this.navigationManager = new window.ReviewModules.NavigationManager();
    ...
    console.log('ReviewAudio constructor completed, project:', this.project, '_startCalled:', this._startCalled)
  }
  ...
  _setupExitWarning() {
    ...
    if (exitBtn) {
      ...
      if (!warningEl) {
        ...
        warningEl.textContent = window.ReviewModules?.WARNING_MESSAGE || 'Please submit your review before exiting.';
  ...
  async handleExit() {
    ...
    // Check if audio has been played but review not submitted
    if (this._audioPlayed && !this._reviewSubmitted) {
      // Show warning message above exit button
  ...
  async _trackAudioPlayed(fileId) {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('Failed to track audio playback:', response.statusText)
      }
    } catch (error) {
  ...
  _setupNavigationPrevention() {
    ...
    this._popStateHandler = (e) => {
      if (this._audioPlayed && !this._reviewSubmitted) {
        ...
        // Show warning
        if (confirm('You have played audio but haven\'t submitted your review yet. Going back will lose your progress. Are you sure you want to leave?')) {
  ...
  // Allow explicit exit (call this if you add an exit button)
  allowExit() {
    this._audioPlayed = false
    this._reviewSubmitted = false
    window.removeEventListener('beforeunload', this._beforeUnloadHandler)
  ...
  autostart() { 
    console.log('ReviewAudio.autostart() called, returning false')
    return false; 
  }
  
  // Override loaded() to prevent autoplay
  loaded() {
    console.log('ReviewAudio.loaded() called, checking autostart()')
    if (this.autostart && this.autostart()) {
      // Only autoplay if autostart() returns true
      super.loaded()
    } else {
      // Don't autoplay - just mark as ready
      console.log('ReviewAudio: Skipping autoplay, waiting for user interaction')
      if (this.ready) {
        this.ready()
      }
    }
  ...
  initialize() {
    console.log('ReviewAudio.initialize() called')
  ...
  // Override prefetch() to filter out non-URL fields before prefetching
  // The parent's prefetch() tries to fetch all string values as URLs,
  // so we need to filter out fields like error, name, answer, next (object), etc.
  // IMPORTANT: The parent's result() does Object.assign(data, {0: url}) which overwrites
  // the new cur URL with the old one. We need to preserve the new cur URL.
  prefetch(next) {
    console.log('ReviewAudio.prefetch() called with:', next)
    // Filter to only include URL-related fields
    // The parent's prefetch() iterates over all keys and tries to fetch string values
    const filtered = {};
    
    // CRITICAL FIX: The parent's result() does Object.assign(data, {0: oldUrl})
    // which overwrites data.cur with the old URL. We need to check if 'cur' exists
    // in the original data and use that instead of the overwritten key 0.
    // But since we're in prefetch(), we don't have access to the original data.
    // However, we can check if key 0 looks like it's the old URL (blob URL) vs new (remote URL)
    // Actually, the parent passes us the merged object, so key 0 might be the old URL.
    // We need to preserve 'cur' if it exists separately, or check if we should use it.
    
    // CRITICAL: The parent's result() does Object.assign(data, {0: oldUrl})
    // which means key 0 has the OLD URL, but 'cur' has the NEW URL from the API.
    // We MUST use 'cur' as the new file URL, not key 0.
    if (next.cur && typeof next.cur === 'string' && next.cur !== '') {
      // The new URL is in 'cur', use it as key 0 (this is the file we want to play next)
      filtered[0] = next.cur;
      console.log('ReviewAudio.prefetch() - Using new cur URL as key 0:', next.cur, '(old key 0 was:', next[0], ')')
    } else if (next[0] && typeof next[0] === 'string' && next[0] !== '') {
      // Fallback: if 'cur' doesn't exist, use key 0 (but this might be the old URL)
      filtered[0] = next[0];
      console.log('ReviewAudio.prefetch() - Using key 0 as fallback:', next[0])
    }
    
    // Preserve only numeric keys (0, 1, 2, etc.) which contain actual URLs
    // The parent's result() method does Object.assign(data, {0: url}), so we get:
    // - 0: current URL (from parent - might be old URL!)
    // - 1, 2, etc.: next URLs (from data.next object)
    // - next: {1: "url", 2: "url"} (object - should NOT be included)
    // - cur: "url" (string - the NEW URL we want to use!)
    // - error, name, answer, etc.: non-URL fields (should NOT be included)
    for (const key in next) {
      const numKey = parseInt(key);
      // Only keep numeric keys (0, 1, 2, etc.) - these contain actual URLs
      // Do NOT include 'next' (it's an object), 'cur' (we handle it above), or other fields
      if (!isNaN(numKey) && numKey.toString() === key) {
        // Only include if it's a string URL (not an object or empty)
        const value = next[key];
        if (typeof value === 'string' && value !== '') {
          // Don't overwrite key 0 if we already set it from 'cur'
          if (key !== '0' || !filtered[0]) {
            filtered[key] = value;
          }
        }
    ...
    console.log('ReviewAudio.prefetch() - Filtered data for parent:', filtered)
  ...
  src(url) {
    console.log('ReviewAudio.src() called with:', url, 'Type:', typeof url, 'Project:', this.project)
    const strUrl = String(url || '')
    
    // Block empty strings, project name, or "#playing" selector
    if (!strUrl || strUrl === "" || strUrl === this.project || strUrl === "#playing") {
      console.error('ReviewAudio.src() - BLOCKED invalid audio URL:', url)
      return
    }
    
    // Block exact invalid paths (but allow valid file paths like /jnd/api/review/upload/...)
    const blockedPaths = [
      `/review`,
      `/review/`,
      `/jnd/api/review`,
    ...
    if (blockedPaths.includes(strUrl)) {
      console.error('ReviewAudio.src() - BLOCKED invalid review path:', url)
      return
    }
    ...
    if (typeof url === 'string' && (url.startsWith('/') || url.startsWith('http') || url.startsWith('blob:'))) {
      // CRITICAL: Stop and reset the audio element before setting new src
      ...
      if (audioElement) {
        ...
        // If we're changing to a different URL, stop the old one first
        const currentSrc = audioElement.src
        if (currentSrc && currentSrc !== url && currentSrc !== '') {
          console.log('Stopping old audio (src:', currentSrc, ') before loading new one (src:', url, ')')
      ...
      console.log('ReviewAudio.src() - Setting valid URL:', url)
      super.src(url)
    } else {
      console.error('ReviewAudio.src() - BLOCKED invalid audio URL format:', url)
    }
  }

  start(project) {
    console.log('ReviewAudio.start() called with project:', project, 'this.project:', this.project)
    // Handle case where project is passed as argument or use instance property
    const projectToUse = project || this.project || "review"
    console.log('ReviewAudio.start() - using project:', projectToUse)
    
    // Check if already called - use strict equality to handle undefined
    if (this._startCalled === true) {
      console.log('start() already called, skipping duplicate call')
    ...
    // No longer using subject selection - get random file from all patient test-type uploads
    // Use absolute path to avoid relative URL issues
    const url = `/jnd/api/${this.project}/start`
    console.log('ReviewAudio.start() - Making API call to:', url)
    ...
    // But we also need to handle the case where the parent's load() might not work correctly
    console.log('ReviewAudio.start() - About to fetch:', url)
    return fetch(url)
      .then(response => {
        console.log('ReviewAudio.start() - API response received, status:', response.status, 'ok:', response.ok)
        if (!response.ok) {
          console.error('ReviewAudio.start() - API response not OK')
        ...
        console.log('ReviewAudio.start() - Returning response to parent')
        return response
      })
      .catch((error) => { 
        console.error('ReviewAudio.start() - API request failed:', error)
  ...
  initialize() {
    if (this._initialized) {
      console.log('initialize() already called, skipping duplicate call')
      return
    }
    ...
    // Only call start() if it hasn't been called already (e.g., by parent constructor)
    ...
    // Set up exit button (already done in constructor, but ensure it's done)
    if (!this.exitManager) {
      this._setupExitButton();
    }
  }
  ...
  _handlePlaybackButtonClick() {
    ...
    if (!audio?.src) {
      console.warn('No audio src available');
      return;
    }
    
    // Users can only play once - if audio has ended, prevent replay
    if (audio.ended) {
      console.log('Audio has already ended - cannot replay');
      return;
    }
    ...
    if (audio.paused) {
      ...
      audio.play().then(() => {
        ...
        alert('Audio not accessible - check console for details');
      });
    }
  }
  ...
  load(data) {
    console.log('ReviewAudio.load() called with data:', data);
    ...
    if (this.loadHandler) {
      this.loadHandler.handleFileLoad(data, this).catch(error => {
        console.error('Error in loadHandler.handleFileLoad:', error);
      });
    } else {
    ...
    // Call parent's load() if it exists
    if (super.load) {
      super.load(data);
    }
  }
  ...
  _loadFallback(data) {
    // Simplified fallback - just the essential logic
    ...
    if (audioElement && data.cur && this.audioPrefetcher) {
  ...
  setupAudioProgress() {
    ...
    if (this.audioEventHandler) {
      ...
      // Fallback: setup event handlers directly
...
window.addEventListener('unhandledrejection', (event) => {
  ...
  console.trace('Rejection stack:');
});

// Create ReviewAudio instance
console.log('About to create ReviewAudio instance');
let audio = new ReviewAudio("review");
console.log('ReviewAudio instance created:', audio);
